...
export async function GET(request: Request) {
  ...
  for (const item of items) {
    const month = (item.attempted_on ?? item.confirmed_at).slice(0, 7);
    trendMap.set(month, (trendMap.get(month) ?? 0) + 1);
    const key = `${item.question_type}|||${item.primary_cause}`;
  ...
  const data: InsightData = {
    ...
    matrix: [...matrixMap.entries()].map(([key, count]) => {
      const [question_type, cause] = key.split("|||");
      return { question_type, cause, count };
    })
  };
